...
export default function SweetsPage() {
  ...
  // Store selected weight per product
  const [weights, setWeights] = useState({});

  useEffect(() => {
    async function fetchProducts() {
      try {
        const res = await fetch("/api/products?category=sweets");
        const data = await res.json();

        // ✅ Normalize API response (array or { products })
  ...
  const calculatePrice = (basePrice, weight) => {
    ...
    return basePrice; // 1kg
  };

  return (
    <div className="min-h-screen bg-gray-50 px-6 py-10 max-w-6xl mx-auto">
      <h1 className="text-3xl font-bold mb-6">Our Sweets</h1>
              ...
              <CardContent className="p-0">
                ...
                <div className="p-3">
                  ...
                  <p className="text-orange-600 font-bold mt-2">
                    ₹{calculatePrice(sweet.price, weights[sweet._id])}
                  </p>
